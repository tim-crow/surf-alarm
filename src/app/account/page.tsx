            ...
            <>
              ...
              <div className="glass-dark p-4 rounded-lg">
                ...
                {(() => {
                  ...
                  const isActive = (opt: typeof tideOptions[0]) => formData.minTide <= opt.min && formData.maxTide >= opt.max;
                  const togglePreset = (preset: typeof tideOptions[0]) => {
                    ...
                    setFormData(prev => ({
                      ...prev,
                      minTide: Math.min(...next.map(a => a.min)),
                      maxTide: Math.max(...next.map(a => a.max)),
                    }));
                  };
                  ...
                  return (
                    <>
                      <div className="grid grid-cols-4 gap-2">
                        ...
                        <button
                          type="button"
                          onClick={() => setFormData(prev => ({ ...prev, minTide: 0, maxTide: 2.5 }))}
                          className={`py-2 px-3 rounded-lg text-sm font-medium transition-all ${
                            allActive
                              ? 'bg-gradient-to-r from-blue-500 to-teal-500 text-white'
                              : 'bg-white/5 text-white/60 hover:bg-white/10 border border-white/10'
                      ...
                      <p className="text-white/40 text-xs mt-2">
                        {allActive ? 'Any tide' : `${formData.minTide.toFixed(1)}m – ${formData.maxTide.toFixed(1)}m`}
                      </p>
                    </>
                  );
                })()}
